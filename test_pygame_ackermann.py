...
def wrap_impl(input_angle):
    ...
    return output_angle
def wrap(angle):
    return r2d(wrap_impl(d2r(angle)))
    ...
    if abs(math.dist((ROBOT_X, ROBOT_Y), coord)) <= dist_tolerance and abs(wrap(ROBOT_TH - (phi_ref if angle_desired == None else angle_desired))) <= angle_tolerance:# and abs((180-ROBOT_TH) - (180-phi_ref)) <= 5:
       ...
       if angle_desired != None:
            ...
            if abs(alpha) < abs(beta):
                ...
                print ("FIRsT branch")
                #ori_err = wrap(angle_desired - ROBOT_TH)
            else:
       ...
       ori_integral = (ori_integral + ori_err) if abs(ori_err) <= angle_tolerance else 0
       ori_deriv = ori_err - last_ori_err
       # ===========
       
       v = 5 * dist + .01*pos_integral + 10*pos_deriv
       delta = wrap(12*ori_err + .001*ori_integral + 10*ori_deriv) #23.515
       ...
       if already_close_to_targ:
            ...
            delta = 5*r2d(math.atan(math.tan(d2r(ori_err)))) #+ .001*ori_integral + 20*ori_deriv
            #print(v)
            
       delta = delta if abs(delta) < 30 else 30 * math.copysign(1, delta)
       ...
       dq = [v*math.cos(d2r(ROBOT_TH)), v*math.sin(d2r(ROBOT_TH)), v*math.tan(d2r(delta))/ROBOT_L]
       noise = 0.001
       ROBOT_X = ROBOT_X + (1/FPS)*dq[0] + random.randrange(1,3)*noise
       ROBOT_Y = ROBOT_Y + (1/FPS)*dq[1] + random.randrange(1,3)*noise
       ROBOT_TH_RAD = ROBOT_TH_RAD + (1/FPS)*dq[2] + random.randrange(1,3)*noise
       ROBOT_TH = wrap(r2d(d2r(ROBOT_TH) + (1/FPS)*dq[2] + random.randrange(1,3)*noise))
       ROBOT_DELTA = wrap(delta) # steering angle (rate)
       
       center_y = ROBOT_L * math.tan((math.pi / 2) - d2r(ROBOT_DELTA))
       left_dist = center_y - ROBOT_W/2
       right_dist = center_y + ROBOT_W/2
       inv_circ = 1 / (math.pi * WHEEL_W)
       
       gain = (2 * math.pi) * v / abs(center_y)
       r = math.sqrt(left_dist ** 2 + ROBOT_L ** 2)
       left_front_ang_vel = gain * r * inv_circ
       r = math.sqrt(right_dist ** 2 + ROBOT_L ** 2)
       right_front_ang_vel = gain * r * inv_circ
       
       gain = (2 * math.pi) * v / center_y
       left_rear_ang_vel = gain * left_dist * inv_circ
       right_rear_ang_vel = gain * right_dist * inv_circ
       
       #ROBOT_DELTA_L = r2d(math.arctan(2 * ROBOT_L * math.tan(d2r(ROBOT_DELTA)) / (2 * ROBOT_L - ROBOT_W * math.tan(d2r(ROBOT_DELTA)))))
        
       arrow_to(actual_center, ROBOT_DELTA, (0, 255, 0), (0, 255, 0))
       if angle_desired == None:
       ...
       print(ROBOT_X, ROBOT_Y, ROBOT_TH, ROBOT_DELTA, ori_err)
       #print(delta, ori_err, ori_integral, ori_deriv)
       #print(delta, v, abs(math.dist((ROBOT_X, ROBOT_Y), coord)), abs(ROBOT_TH - phi_ref))
       #print(left_front_ang_vel, right_front_ang_vel, left_rear_ang_vel, right_rear_ang_vel)
